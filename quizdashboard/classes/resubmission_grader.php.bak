...
class resubmission_grader extends essay_grader {
    ...
    private function extract_previous_scores($previous_grading_record) {
        ...
        $sections = [
            'content_and_ideas' => [
                ...
                'pattern' => '/Content and Ideas.*?Score:.*?(\d+)\s*\/\s*25/si', 
                'max' => 25
            ],
            'structure_and_organization' => [
                ...
                'pattern' => '/Structure and Organization.*?Score:.*?(\d+)\s*\/\s*25/si', 
                'max' => 25
            ],
            'language_use' => [
                'marker' => 'LANGUAGE_USE',
                ...
                'pattern' => '/Language Use.*?Score:.*?(\d+)\s*\/\s*20/si', 
                'max' => 20
            ],
            'creativity_and_originality' => [
                ...
                'pattern' => '/Creativity and Originality.*?Score:.*?(\d+)\s*\/\s*20/si', 
                'max' => 20
            ],
            'mechanics' => [
                'marker' => 'MECHANICS',
                ...
                'pattern' => '/Mechanics.*?Score:.*?(\d+)\s*\/\s*10/si', 
                'max' => 10
            ]
        ];
        ...
        } else if (!$final_score_found && preg_match('/Final Score \\(Previous.*?→.*?New\\):.*?\\d+\\s*\\/\\s*100\\s*→\\s*(\\d+)\\s*\\/\\s*100/si', $feedback_html, $matches)) {
            $scores['final_score'] = ['score' => (int)$matches[1], 'max' => 100];
    ...
    private function generate_penalty_feedback($previous_scores, $submission_number) {
        ...
        foreach ($sections as $key => $config) {
            ...
            $html .= "<p><strong>Score (Previous → New):</strong> {$prev_score}/{$config['max']} → 0/{$config['max']}</p>";
            $html .= "<ul>";
...
<br>â€¢ <span style=\"color:#808080;\">Original: [student's text in grey]</span>
<br>â€¢ <span style=\"color:#3399cc;\">Improved: [corrected version in blue]</span>
