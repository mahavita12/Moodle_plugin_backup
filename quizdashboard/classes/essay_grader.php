...
class essay_grader {
    ...
    private const API_CONNECT_TIMEOUT = 30;  // 30 seconds to connect
    private const API_TOTAL_TIMEOUT = 180;   // 3 minutes total (reduced from 4 minutes)
    private const MAX_RETRY_ATTEMPTS = 2;    // Retry failed API calls
    ...
    protected function save_grade_to_moodle(array $essay_data, array $feedback_data): void {
        ...
        else if (preg_match('/<strong>Final Score \\(Previous.*?→.*?New\\):.*?→\\s*(\\d+(?:\\.\\d+)?)\\s*\\/\\s*(\\d+(?:\\.\\d+)?)<\\/strong>/i', $feedback_data['feedback_html'], $matches)) {
        ...
        if (!is_numeric($fraction) || $fraction < 0.0 || $fraction > 1.0) {
            ...
            $fraction = 0.0; // Default to 0 if invalid
        }
        ...
        while ($attempts < self::MAX_RETRY_ATTEMPTS) {
            ...
            try {
                ...
                if (isset($body['error'])) {
                    ...
                    if ($attempts < self::MAX_RETRY_ATTEMPTS) {
                        sleep(5 * $attempts); // Longer wait for API errors
                        continue;
                    }
