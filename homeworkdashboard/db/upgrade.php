...
function xmldb_local_homeworkdashboard_upgrade($oldversion) {
    ...
    // Book Reading Tracker table
    if ($oldversion < 2026030300) {
        $dbman = $DB->get_manager();

        $table = new xmldb_table('local_hw_books');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('title', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '');
        $table->add_field('author', XMLDB_TYPE_CHAR, '255', null, null, null, '');
        $table->add_field('pages_read', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('finished', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('points_awarded', XMLDB_TYPE_NUMBER, '10, 2', null, XMLDB_NOTNULL, null, '100.00');
        $table->add_field('adjustment_id', XMLDB_TYPE_INTEGER, '10', null, null, null, null);
        $table->add_field('week_ending', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('courseid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('createdby', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('timecreated', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_index('idx_userid', XMLDB_INDEX_NOTUNIQUE, ['userid']);
        $table->add_index('idx_userid_week', XMLDB_INDEX_NOTUNIQUE, ['userid', 'week_ending']);
        $table->add_index('idx_adjustment', XMLDB_INDEX_NOTUNIQUE, ['adjustment_id']);

        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        upgrade_plugin_savepoint(true, 2026030300, 'local', 'homeworkdashboard');
    }
