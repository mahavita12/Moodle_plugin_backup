...
// Fetch books for this student and week
$books_data = $DB->get_records('local_hw_books', [
    'userid' => $userid, 'week_ending' => $timeclose
], 'timecreated ASC');
$books_for_ai = [];
foreach ($books_data as $bk) {
    $books_for_ai[] = [
        'title' => $bk->title,
        'author' => $bk->author,
        'finished' => (int)$bk->finished,
        'points' => (float)$bk->points_awarded
    ];
}
