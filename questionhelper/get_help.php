...
if (!get_config('local_questionhelper', 'enabled') || !local_questionhelper_is_configured()) {
    ...
    echo json_encode(['success' => false, 'error' => 'Service not available']);
    exit;
}

try {
    $response = call_openai($questiontext, $options);
 ...
 */
function call_openai($questiontext, $options) {
    ...
    $prompt = build_prompt($questiontext, $options);
