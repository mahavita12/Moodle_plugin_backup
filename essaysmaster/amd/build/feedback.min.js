...
define([], function () {
    ...
    // Attempt id for popup naming
    let __EM_ATTEMPT_ID__ = null;

    // Add a small external-link icon to the panel and open a popup mirroring it
    function addOpenWindowButton(panel, round, attemptId) {
        try {
            if (!panel || !round) return;
            if (panel.querySelector('.em-openwin')) return; // avoid duplicates
            ...
            btn.style.cssText = [
                ...
                'border:0','background:rgba(255,255,255,0.0)','cursor:pointer','opacity:0.85','padding:0','line-height:0'
            ].join(';');
            ...
            const popupBtn=document.createElement('button'); popupBtn.type='button'; popupBtn.className='em-openpopup'; popupBtn.setAttribute('aria-label','Open in new window'); popupBtn.style.cssText=['position:absolute','top:8px','right:30px','width:18px','height:18px','border:0','background:rgba(255,255,255,0.0)','cursor:pointer','opacity:0.85','padding:0','line-height:0'].join(';'); popupBtn.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0f6cbf" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="7" width="12" height="12" rx="2"/><rect x="9" y="3" width="12" height="12" rx="2"/></svg>';
            ...
            btn.addEventListener('click', function(ev){
                ...
                container.style.cssText = ['position:fixed','top:10px','right:10px','width:'+width+'px','max-height:85vh','background:#fff','border:2px solid #0f6cbf','border-radius:8px','box-shadow:0 8px 24px rgba(0,0,0,0.2)','z-index:999999','display:flex','flex-direction:column','overflow:hidden','font-family: Arial, Helvetica, sans-serif','font-size:14px'].join(';');
                const header = document.createElement('div'); header.style.cssText='cursor:move;background:#0f6cbf;color:#fff;padding:6px 10px;display:flex;justify-content:space-between;align-items:center;'; header.innerHTML='<span>Feedback Round ' + round + '</span>';
                const closeBtn = document.createElement('button'); closeBtn.type='button'; closeBtn.textContent='×'; closeBtn.style.cssText='background:transparent;border:0;color:#fff;font-size:18px;line-height:1;cursor:pointer';
                const tools=document.createElement('div'); tools.style.cssText='margin-left:auto;display:flex;gap:6px;align-items:center;'; const maxBtn=document.createElement('button'); maxBtn.type='button'; maxBtn.textContent='▢'; maxBtn.title='Maximize/Restore'; maxBtn.style.cssText='background:transparent;border:1px solid rgba(255,255,255,.7);color:#fff;font-size:12px;line-height:1;padding:2px 6px;border-radius:3px;cursor:pointer'; tools.appendChild(maxBtn); header.appendChild(tools); header.appendChild(closeBtn);
                const body = document.createElement('div'); body.style.cssText='padding:10px;overflow:auto';
                const clone = panel.cloneNode(true); const existingBtn = clone.querySelector('.em-openwin'); if (existingBtn) existingBtn.remove(); body.appendChild(clone);
                container.appendChild(header); container.appendChild(body); document.body.appendChild(container); window.__EM_FLOAT_PANEL__ = container;
                const skey='em_fp_'+(__EM_ATTEMPT_ID__||attemptId||'0');
                (function(){ try { var raw=sessionStorage.getItem(skey); if(!raw) return; var st=JSON.parse(raw)||{}; if(st.width) container.style.width=Math.max(360, parseInt(st.width))+'px'; if(st.height) container.style.height=Math.max(240, parseInt(st.height))+'px'; if(typeof st.top==='number') container.style.top=st.top+'px'; if(typeof st.right==='number') container.style.right=st.right+'px'; } catch(_){} })();
                function persist(){ try { var st={ top: container.offsetTop, right: parseInt(container.style.right)||10, width: container.offsetWidth, height: container.offsetHeight }; sessionStorage.setItem(skey, JSON.stringify(st)); } catch(_){} }
                (function(){ let dragging=false, sx=0, sy=0, startTop=0, startRight=0; header.addEventListener('mousedown', function(e){ dragging=true; sx=e.clientX; sy=e.clientY; startTop=container.offsetTop; startRight=parseInt(container.style.right)||10; e.preventDefault(); }); document.addEventListener('mousemove', function(e){ if(!dragging) return; const dy=e.clientY-sy; const dx=e.clientX-sx; container.style.top=(startTop+dy)+'px'; container.style.right=(Math.max(0,startRight-dx))+'px'; }); document.addEventListener('mouseup', function(){ if(dragging) persist(); dragging=false; }); })();
                function addHandle(dir, css, cursor){ const h=document.createElement('div'); h.style.cssText='position:absolute;'+css+';cursor:'+cursor+';'; container.appendChild(h); let resizing=false, sx=0, sy=0, startW=0, startH=0, startTop=0, startRight=0; h.addEventListener('mousedown', function(e){ resizing=true; sx=e.clientX; sy=e.clientY; startW=container.offsetWidth; startH=container.offsetHeight; startTop=container.offsetTop; startRight=parseInt(container.style.right)||10; e.preventDefault(); }); document.addEventListener('mousemove', function(e){ if(!resizing) return; const dx=e.clientX-sx, dy=e.clientY-sy; let w=startW, h=startH, t=startTop, r=startRight; if(dir.indexOf('e')!==-1){ w=Math.max(360,startW+dx); r=Math.max(0,startRight-dx); } if(dir.indexOf('s')!==-1){ h=Math.max(240,startH+dy); } if(dir.indexOf('w')!==-1){ w=Math.max(360,startW-dx); r=Math.max(0,startRight+dx); } if(dir.indexOf('n')!==-1){ h=Math.max(240,startH-dy); t=startTop+dy; } container.style.width=w+'px'; container.style.height=h+'px'; container.style.top=t+'px'; container.style.right=r+'px'; }); document.addEventListener('mouseup', function(){ resizing=false; }); }
                ...
                let maximized=false, prev={ top:0, right:0, width:0, height:0 }; maxBtn.addEventListener('click', function(){ if(!maximized){ prev.top=container.offsetTop; prev.right=parseInt(container.style.right)||10; prev.width=container.offsetWidth; prev.height=container.offsetHeight; container.style.top='10px'; container.style.right='10px'; container.style.width=Math.min(window.innerWidth-20, 1000)+'px'; container.style.height=Math.min(window.innerHeight-20, 700)+'px'; maximized=true; maxBtn.textContent='❐'; } else { container.style.top=prev.top+'px'; container.style.right=prev.right+'px'; container.style.width=prev.width+'px'; container.style.height=prev.height+'px'; maximized=false; maxBtn.textContent='▢'; } persist(); });
            ...
            // Split-screen popup (two-window button)
            popupBtn.addEventListener('click', function(ev){
                ...
                if (w) {
                    ...
                    const styles = (function(){ var el = document.getElementById('essays-master-styles'); return el ? el.textContent : ''; })();
                    const clone = panel.cloneNode(true); var existingBtn = clone.querySelector('.em-openwin'); if (existingBtn) existingBtn.remove();
                    const html = clone.outerHTML;
                    const payload = { type: 'em_feedback_clone', title: 'Feedback Round ' + round, styles: styles, html: html };
                    const send = function(){ try { w.postMessage(payload, '*'); } catch(_){} };
                    setTimeout(send, 250);
        ...
        } catch (e) { console.warn('Essays Master: open window button failed', e); }
    }
    ...
    function handleValidation(panel, round, previousRound, btn, attemptId) {
        ...
        getValidationFeedback(round, previousContent, currentContent, questionPrompt)
            .then(result => {
                ...
                addOpenWindowButton(panel, round, attemptId);
            })
            .catch(error => {
    ...
    function init(options) {
        ...
        const boot = () => {
            ...
            btn.addEventListener('click', async function handle(e) {
                ...
                processing = true; try { if (window.__EM_FEEDBACK_WIN__ && !window.__EM_FEEDBACK_WIN__.closed) { window.__EM_FEEDBACK_WIN__.close(); } } catch(_) {} try { window.__EM_FEEDBACK_WIN__ = null; } catch(_) {}
                e.preventDefault();
                e.stopPropagation();
        ...
        try { window.addEventListener('beforeunload', function(){ try { if (window.__EM_FLOAT_PANEL__ && window.__EM_FLOAT_PANEL__.parentNode) { window.__EM_FLOAT_PANEL__.parentNode.removeChild(window.__EM_FLOAT_PANEL__); } } catch(_) {} try { if (window.__EM_FEEDBACK_WIN__ && !window.__EM_FEEDBACK_WIN__.closed) { window.__EM_FEEDBACK_WIN__.close(); } } catch(_) {} }); } catch(_) {}
    }
