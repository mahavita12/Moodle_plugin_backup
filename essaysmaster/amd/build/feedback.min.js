...
define([], function () {
    ...
    const ROUND_CONFIGS = {
        1: { type: 'feedback', title: 'GrowMinds Academy Feedback', focus: 'Grammar, Spelling & Punctuation' },
        2: { type: 'validation', title: 'GrowMinds Academy Validation', focus: 'Proofreading Validation' },
        3: { type: 'feedback', title: 'GrowMinds Academy Feedback', focus: 'Language Enhancement' },
        4: { type: 'validation', title: 'GrowMinds Academy Validation', focus: 'Language Validation' },
        5: { type: 'feedback', title: 'GrowMinds Academy Feedback', focus: 'Relevance & Structure' },
        6: { type: 'validation', title: 'GrowMinds Academy Validation', focus: 'Final Validation' }
    };

    function getRoundConfig(round) {
        return ROUND_CONFIGS[round] || { type: 'feedback', title: 'GrowMinds Academy Feedback', focus: 'Essay Review' };
    }
    ...
    function formatFeedbackParagraphs(text, round) {
        ...
        formatted.forEach((item, index) => {
            if (round === 1 && index === formatted.length - 1) {
                item.extraClass = (item.extraClass || '') + ' feedback-line-emphasis';
            }
        });
    ...
    function handleValidation(panel, round, previousRound, btn, attemptId) {
        ...
        getValidationFeedback(round, previousContent, currentContent, questionPrompt)
            .then(result => {
                ...
                if (result.success) {
                    validationContent += `<div class="essay-emphasis-box" style="color:#28a745;">${mainFeedback.replace(/\n/g, '<br>')}</div>`;
                    ...
                    validationContent += `<div class="essay-emphasis-box" style="color:#dc3545;">${mainFeedback.replace(/\n/g, '<br>')}</div>`;
